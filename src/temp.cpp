...
std::vector<float> Relocalizer::calcLocation(cv::Mat query_img) {
  ...
  std::vector<float> resDefault;

  resDefault.push_back(-1.0);
  resDefault.push_back(-1.0);
  resDefault.push_back(0.0);
    ...
    cout << "Size matches: " << matches.size() << " and sizeof " << sizeof(matches) << endl;
      ...
      if (homography.cols > 0) {
          ...
          center_transformed.x = (center_transformed.x/ref_img.cols);
          center_transformed.y = (center_transformed.y/ref_img.rows);
            ...
            res.push_back(center_transformed.x);
            res.push_back(center_transformed.y);
            res.push_back(matches.size());
...
boost::python::list Relocalizer::calcLocationFromPath(std::string query_img_path) {
  ...
  python_list.append(loc[0]);
  python_list.append(loc[1]);
  python_list.append(loc[2]);
